...
interface LeaderboardCategoryProps {
   ...
   type: LeaderboardType; // Ensure this is the correct type
   setType: React.Dispatch<React.SetStateAction<LeaderboardType>>; // Use the correct type here
}
      ...
      <aside className="w-full lg:w-1/4 bg-neutral-700/50 backdrop-blur-xl border border-neutral-600 rounded-lg p-4 h-fit mb-4 lg:mb-0">
         ...
         <ul>
            {categories.map(category => (
               <li key={category}>
                  <button
                     onClick={() => setType(category)} // category is already of type LeaderboardType
                     className={`block w-full mb-2 text-left p-2 rounded-md hover:bg-neutral-600 text-white ${type === category ? 'bg-neutral-600' : ''}`}>
